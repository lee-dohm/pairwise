...
class PairwiseTests < Test::Unit::TestCase
  ...
  def test_generate_all_pairs
    pairs = Pairwise.generate_all_pairs(@left, @right)
    
    assert_equal 9, pairs.count, 'The wrong number of elements are in the list of pairs'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:a, :a)), 'The a-a pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:a, :b)), 'The a-b pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:a, :c)), 'The a-c pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:b, :a)), 'The b-a pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:b, :b)), 'The b-b pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:b, :c)), 'The b-c pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:c, :a)), 'The c-a pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:c, :b)), 'The c-b pair is not included in the list'
    assert_equal true, pairs.include?(Pairwise::TestPair.new(:c, :c)), 'The c-c pair is not included in the list'
  end
  ...
  def test_generate_all_pairs_raises_when_supplied_with_an_empty_left_list
    assert_raise ArgumentError do
      Pairwise.generate_all_pairs([], @right)
    end
  end
  ...
  def test_generate_all_pairs_raises_when_supplied_with_an_empty_right_list
    assert_raise ArgumentError do
      Pairwise.generate_all_pairs(@left, [])
    end
  end
  ...
  def test_generate_all_pairs_raises_when_left_is_nil
    assert_raise ArgumentError do
      Pairwise.generate_all_pairs(nil, @right)
    end
  end
  ...
  def test_generate_all_pairs_raises_when_right_is_nil
    assert_raise ArgumentError do
      Pairwise.generate_all_pairs(@left, nil)
    end
  end
  ...
  def test_generate_all_pairs_raises_when_there_is_only_one_item_in_left_list
    assert_raise ArgumentError do
      Pairwise.generate_all_pairs(@left.take(1), @right)
    end
  end
  ...
  def test_generate_all_pairs_raises_when_there_is_only_one_item_in_right_list
    assert_raise ArgumentError do
      Pairwise.generate_all_pairs(@left, @right.take(1))
    end
  end
end
